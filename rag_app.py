...
try:
    ...
    # 5. Kullanıcıdan sürekli soru al
    while True:
        question = input(
            "\nPDF hakkında bir soru sor "
            "(çıkmak için 'çıkış' yaz): "
        ).strip()

        if question.lower() in ["çıkış", "cikis", "exit", "quit"]:
            print("\nProgram kapatılıyor...")
            break

        if not question:
            print("Lütfen boş soru girmeyin.")
            continue

        # Önceki sorudan sonra kaldırıldıysa embedding modelini yeniden yükle.
        if not embedding_loaded:
            print("\nEmbedding modeli yeniden yükleniyor...")
            embedding_model.load()
            embedding_loaded = True
            embedding_client = embedding_model.get_embedding_client()
        ...
        if not candidate_sentences:
            print("\nİlgili parçalardan cevap adayı cümle çıkarılamadı.")
            continue
        ...
        # 9. Phi-4 Mini için belleği boşalt
